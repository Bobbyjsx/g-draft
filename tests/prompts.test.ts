...
describe('PROMPTS', () => {
  it('should include CURRENT ACTION in COMMIT prompt', () => {
    const prompt = PROMPTS.COMMIT('some-diff-path');
    expect(prompt).toContain('CURRENT ACTION: Generating a Commit Message');
    expect(prompt).toContain('some-diff-path');
  });

  it('should include CURRENT ACTION in PR prompts', () => {
    const noTemplate = PROMPTS.PR_NO_TEMPLATE('some-diff-path');
    const withTemplate = PROMPTS.PR_WITH_TEMPLATE('template', 'some-diff-path');
  ...
  it('should include CURRENT ACTION in REVIEW prompt', () => {
    const prompt = PROMPTS.REVIEW('some-diff-path');
  ...
  it('should include custom instructions when provided', () => {
    ...
    const prompt = PROMPTS.COMMIT('some-diff-path', options);
